-- ============================================================
-- EAS AI Adoption — Migration 055: add hierarchy columns to
-- get_org_leaderboard so the Org Rankings card can display the
-- "Sector › Department" breadcrumb directly. This unblocks the
-- removal of the standalone Practice Rankings section (which
-- previously sourced its breadcrumb from get_practice_leaderboard).
--
-- Signature changes — DROP+CREATE.
--   sector tab   → department_name = NULL,   sector_name = self
--   unit tab     → department_name = self,   sector_name = parent
--   practice tab → department_name = parent, sector_name = grandparent
-- ============================================================
...
BEGIN
  ...
  ELSIF p_level = 'unit' THEN
    RETURN QUERY
      WITH agg AS (
        SELECT d.id AS scope_id, d.name AS scope_name, s.name AS sector_name,
          (SELECT count(DISTINCT t.employee_name)::INT FROM tasks t
      ...
      SELECT a.scope_id, a.scope_name, a.contributors, a.tasks, a.hours_saved,
             ...
             ROUND(
               ...
               COALESCE(a.quality_avg, 0) * 2
             , 1) AS score,
             a.scope_name AS department_name,
             a.sector_name
      FROM agg a
      ORDER BY score DESC NULLS LAST;
  ...
  ELSIF p_level = 'practice' THEN
    RETURN QUERY
      WITH agg AS (
        SELECT p.id AS scope_id, p.name AS scope_name, d.name AS department_name, s.name AS sector_name,
          (SELECT count(DISTINCT t.employee_name)::INT FROM tasks t
