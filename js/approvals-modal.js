...
const ApprovalsModal = (() => {
  ...
  async function fetchQueue() {
    ...
    try {
      let items = await EAS_DB.fetchPendingApprovals(role, profile.practice, profile.id);
      // Pop-up is role-aware: each approver only sees items actionable at THEIR
      // layer. fetchPendingApprovals intentionally returns broader visibility
      // lists for admin (all pending layers) — narrow that here so the admin
      // pop-up counts only admin-stage items. SPOC/dept_spoc/team_lead queries
      // are already scoped to spoc_review for their practice/department/team.
      if (role === 'admin') {
        items = (items || []).filter(a => ['pending', 'admin_review'].includes(a.approval_status));
      }
