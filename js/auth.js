...
const EAS_Auth = (() => {
  ...
  function isAiInnovationReviewer() {
    return _userProfile?.role === 'ai_innovation_reviewer';
  }

  function canReviewUseCases() {
    return isAdmin() || isAiInnovationReviewer();
  }
  ...
  function updateUserDisplay() {
    ...
    if (roleEl) {
      const roleLabels = { admin: 'Administrator', spoc: 'AI SPOC', dept_spoc: 'Dept SPOC', sector_spoc: 'Sector SPOC', contributor: 'Contributor', viewer: 'Viewer', executive: 'Executive', team_lead: 'Team Lead', ai_innovation_reviewer: 'AI Innovation Reviewer' };
...
// Phase D codex review FIX B — expose on window for cross-script access.
if (typeof window !== 'undefined') { window.EAS_Auth = EAS_Auth; }
